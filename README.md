...
If you are interested in re-training a detector, please see [BOP 2020 results]().
...
Notes:
- Be careful about using our ICP refiner, it slightly decrease performance over RGB-only on T-LESS instead of improving the results. Qualitative results show a misalignment of many objects after ICP, there is likely a small bug in the ICP refinement but I haven't had time to go in detail. Note that our method and paper focused on the RGB-only setting.
